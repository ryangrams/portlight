...
for PAIR in i686:x86 x86_64:x64 aarch64:arm64; do
  TARGET=${PAIR%:*}
  LABEL=${PAIR#*:}
  "$LLVM_MINGW/bin/$TARGET-w64-mingw32-g++" -std=c++17 -O2 -DNDEBUG -DUNICODE -D_UNICODE -D_WIN32_WINNT=0x0A00 -DWINVER=0x0A00 -municode -mwindows -static -static-libgcc -static-libstdc++ -Wall -Wextra "$BASE/main.cpp" -o "$BASE/dist/SU-Remote-Viewer-$LABEL.exe" -lwinhttp -lwindowscodecs -lole32 -lcomctl32 -lshlwapi -lshell32 -lcrypt32 -lbcrypt -lws2_32 -lwinmm -luuid
  "$LLVM_MINGW/bin/llvm-strip" "$BASE/dist/SU-Remote-Viewer-$LABEL.exe"
  mkdir -p "$BASE/dist/$LABEL"
  cp "$BASE/dist/SU-Remote-Viewer-$LABEL.exe" "$BASE/dist/$LABEL/SU Remote Viewer.exe"
