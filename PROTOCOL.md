...
## Implemented Mac viewer extensions

The Mac host additionally reports `logicalWidth` and `logicalHeight` in its display list. These are the macOS display bounds in points, in the same coordinate system as `x` and `y`. Clients can fall back to native pixel size divided by `scale` when these optional fields are absent. Layout proportions must not depend on the negotiated stream resolution.

Hosts advertising `aac` in `capabilities.audio` accept optional subscription fields `audioCodec: "aac"` and `audioBitrate` of 48000 (mono), 96000, 160000, or 320000 (stereo). The defaults remain `mulaw` and the existing 24 kHz mono wire format, preserving compatibility with older viewers. New audio choices do not enable audio by themselves.

AAC is AAC-LC at 48 kHz. Each audio binary envelope contains one raw AAC access unit, without ADTS framing. Its JSON header includes `codec: "aac"`, `sampleRate: 48000`, `channels: 1` or `2`, `samples: 1024`, the nominal `bitrate`, and the encoder's `cookie` as a base64 string. The raw access unit follows the usual length-prefixed JSON header. The cookie is decoder initialization metadata, not encoded audio. Receivers must bound the cookie (4096 decoded bytes), access unit (16384 bytes), channels, sample count, and sample rate. Audio still shares the session sequence counter, receives no frame acknowledgements, and is discarded on subscription changes or pause. Playback queues remain bounded.

The Mac viewer requests up to 60 fps and allows the existing host backpressure to reduce delivery. Its displayed fps is the host's changed-image update count averaged over selected displays, not tile count or a guarantee of 60 fps for static content. The default bandwidth budget is Automatic (`bandwidthKbps: 0`).

### September 10 media behavior

New Mac viewer sessions subscribe to all advertised displays immediately, including recalled connections. Subsequent display selections stay within the same session. Hosts can emit standard indexed PNG8 for `color256`, preserving the existing RGB332 palette and decoder compatibility.

Optional subscription boolean `dither` defaults to false. With `quality: "motion"`, true enables a fixed, low-amplitude 8×8 ordered dither for reduced-color modes. It adds no frame buffering, but increases payload size; it is opt-in because additional bandwidth can affect latency. Desktop and Auto keep the established quantization.

Audio has a dedicated capture stream that survives video-only subscription changes when codec/rate remain unchanged. Audio is prioritized ahead of queued images and is outside the video bandwidth limit. Audio queues remain bounded (12 outbound packets); congestion can still drop old audio. TCP head-of-line blocking remains possible. The Mac viewer keeps playback across video revisions, accepts audio from prior revisions in the same active connection, decodes on a dedicated serial queue, and uses about 64 ms of AAC startup buffering. Pausing or turning audio off still stops playback.
