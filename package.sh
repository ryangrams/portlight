...
cp -R licenses "$MAC/"
ditto -c -k --norsrc --noextattr --keepParent "$MAC" "$OUT/SU-Remote-macOS-AppleSilicon-$VERSION.zip"
for ARCH in x86 x64 arm64; do
  ...
  cp -R licenses "$WIN/"
  ditto -c -k --norsrc --noextattr --keepParent "$WIN" "$OUT/SU-Remote-Windows-$ARCH-$VERSION.zip"
done
